...
# ### Используемые ковариационные функции
#
...
kernels = [ GP_kernels.gauss, GP_kernels.rational_quadratic,
            GP_kernels.periodic,  GP_kernels.brownian]
kernel_names = ['Gauss kernel', 'Rational quadratic kernel',
                'Periodic kernel', 'Brownian kernel']
kernel_args = {'l':1.0, 'sigma_k':1.0, 'alpha':0.1, 'period':4.5}
sigma_n = 1e-3
...
# $$
#
# Теперь пришло время поэкспериментировать с параметрами ядра: ширины ядра $l$ и амплитуды $\sigma_k$.
#
...
# Вектор математического ожидания $\mu$ и ковариационная матрица $\Sigma$ зависят от наблюдаемых данных $X$ и параметров ядра $\theta$: $\mu = m(X)$, $\Sigma = k_\theta(X, X)$.
# Для простоты полагаем $m(X) = 0$.

# Оптимальные значения параметров ядра (гиперпараметров) могут быть оценены путём максимизации логарифма правдоподобия
# $$
