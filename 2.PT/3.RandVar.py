...
#
# Пусть и $(\Omega, \mathcal{F})$ и $(E, \mathcal{B})$ &mdash; два измеримых пространства.
# Функция $\xi = \xi(\omega)$, определённая на $(\Omega, \mathcal{F})$ со значениями в $E$, называется $\mathcal{F}/\mathcal{B}$-*измеримой*, если множество $\{\omega: \xi(\omega) \in \mathcal{B}\} \in \mathcal{F}$ для всякого $B \in \mathcal{B}$.
# В теории вероятностей такие функции называют *случайными элементами* со значениями в $E$.
# В том случае, когда $E = \mathbb{R}$ &mdash; действительная прямая, а $\mathcal{B}$ &mdash; $\sigma$-алгебра борелевских подмножеств $\mathbb{R}$, $\mathcal{F}/\mathcal{B}$-измеримые функции $\xi=\xi(\omega)$ называют (действительными) случайными величинами.
# В этом специальном случае $\mathcal{F}/\mathcal{B}$-измеримые функции для краткости называют просто $\mathcal{F}$-измеримыми.
#
# Ещё раз другими словами: \
# **Определение.** Функция $\xi: \Omega \rightarrow \mathbb{R}$ называется *случайной величиной*, если для любого борелевского множества $B \in \mathcal{B}(\mathbb{R})$ множество $\xi^{-1}(B)$ является событием, т. е. принадлежит $\sigma$-алгебре $\mathcal{F}$.
#
# И ещё раз другими словами: \
# **Определение.** Функция $\xi: \Omega \rightarrow \mathbb{R}$ называется *случайной величиной*, если для любого $x \in \mathbb{R}$ множество $\{\omega : \xi(\omega) \le x\}$ принадлежит $\sigma$-алгебре $\mathcal{F}$.
#
...
#
# **Определение.** Вероятностная мера $P_\xi$ на $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$ c
# $$ P_\xi(B) = \mathrm{P}\{\omega: \xi(\omega) \in B\}, \quad B \in \mathcal{B}(\mathbb{R}), $$
# называется *распределением вероятностей* случайной величины $\xi$ на $(\mathbb{R}, \mathcal{B}(\mathbb{R}))$.
#
# **Определение.** Функция
...
#
# **Определение.** Случайная величина $\xi$ называется *абсолютно непрерывной*, если существует неотрицательная функция $p=p_\xi(x)$, называемая *плотностью распределения*, такая, что
# $$ F_\xi(x) = \int\limits_{-\infty}^x p_\xi(t) dt, \quad \forall x \in \mathbb{R}. $$
#
# **Свойства:**
#
# 1. $\forall x: p_\xi (x) \ge 0$;
# 1. $\int\limits_{-\infty}^{\infty} p_\xi(t) dt = 1$.
#
# Эти два свойства полностью характеризуют класс плотностей.
#
# **Теорема.** Если функция $p$ обладает свойствами (1) и (2), то существует вероятностное пространство и случайная величина $\xi$ на нём, для которой $p$ является плотностью распределения.
...
# распределение*, если существует неотрицательная функция $p_{\xi, \eta}(x, y)$, называемая *плотностью*, такая, что
# $$ F_{\xi, \eta}(x, y) = \int\limits_{-\infty}^{x} \int\limits_{-\infty}^{y} p_{\xi, \eta}(t_1, t_2) dt_1 dt_2. $$
#
# **Теорема.** Если случайные величины $\xi$ и $\eta$ имеют абсолютно непрерывное совместное распределение с плотностью $p(x, y)$, то $\xi$ и $\eta$ в отдельности также имеют абсолютно непрерывное распределение
# с плотностями:
# $$ p_{\xi}(x) = \int\limits_{-\infty}^{\infty} p_{\xi, \eta}(x, y)dy \quad \mathrm{и} \quad p_{\eta}(y) = \int\limits_{-\infty}^{\infty} p_{\xi, \eta}(x, y)dx. $$
...
# $\forall x_1, \dots , x_n:$
# $$ p_{\xi_1, \dots, \xi_n}(x_1, \dots, x_n) = p_{\xi_1}(x_1) \cdot \ldots \cdot p_{\xi_n}(x_n).$$
...
#
# > Коэффициент корреляции можно интерпретировать как косинус угла в некотором пространстве случайных величин, в котором скалярным произведением является ковариация.
