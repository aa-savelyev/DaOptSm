...
\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}
    ...
    \hypertarget{ux441ux43fux438ux441ux43eux43a-ux43eux431ux43eux437ux43dux430ux447ux435ux43dux438ux439}{%
\subsection{Список
обозначений}\label{ux441ux43fux438ux441ux43eux43a-ux43eux431ux43eux437ux43dux430ux447ux435ux43dux438ux439}}

\((\Omega, \mathcal{F}, \mathbb{P})\) --- вероятностное пространство
(\(\Omega\) --- множество исходов, \(\mathcal{F}\) ---
\(\sigma\)-алгебра, \(\mathbb{P}\) --- вероятностная мера)

\(\mathrm{E}X\), \(\mathrm{E}(X)\) --- математическое ожидание случайной
величины \(X\)

\(\mathrm{D}X\), \(\mathrm{D}(X)\) --- дисперсия случайной величины
\(X\)
...
белого цвета и \(M_2\) --- черного, то какова вероятность \(P(A)\)
...
которых \(M_1 - 1\) шаров имеют белый цвет, а \(M_2\) --- черный;
...
интуитивными представлениями о ней.
    ...
    \emph{Определение 1.} Условной вероятностью события \(B\) при условии
...
предположении, что: a) старший ребенок --- мальчик; b) по крайней мере
...
\textbf{Определение 5.} Пусть \(( \xi, \eta )\) --- пара случайных
...
\subsection{Гауссовские случайные
...
(\emph{Ширяев, параграф 8. Случайные величины. II})

Если \(\xi\) --- случайная величина с гауссовской (нормальной)
плотностью

\[ f_\xi(x) = \frac{1}{\sqrt{2 \pi} \sigma} e^{-(x-m)^2 / 2 \sigma^2}, \quad \sigma>0, \quad -\infty < m < \infty, \]

то смысл параметров \(m\) и \(\sigma\) оказывается очень простым:
\[ m = \mathrm{E} \xi, \quad \sigma^2 = \mathrm{D} \xi . \]

Таким образом, рапределение вероятностей этой случайной величины
\(\xi\), называемой \emph{гауссовской} или \emph{нормально
распределённой}, полностью определяется её средним значением \(m\) и
дисперсией \(\sigma^2\). В этой связи часто используется запись
\(\xi \sim \mathcal{N}\left( m, \sigma^2 \right)\).

\textbf{Свойство:} Если \(\xi\) и \(\eta\) --- гауссовские случайные
величины, то из их \emph{некоррелированности} следует их
\emph{независимость}.

\begin{center}\rule{0.5\linewidth}{\linethickness}\end{center}

    Пусть \(\xi = \left( \xi_1, \dots, \xi_n \right)\) --- случайный вектор,
компоненты которого имеют конечный второй момент. Назовём \emph{матрицей
ковариаций} (ковариационной матрицей) вектора \(\xi\) матрицу (порядка
\(n \times n\)) \(\mathbb{R} = ||R_{ij}||\), где
\(R_{ij} = \text{cov}\left( \xi_i, \xi_j \right)\).

Матрица \(\mathbb{R}\) является \emph{симметрической} и
\emph{неотрицательно определённой}. Следующая лемма показывает, что
справедлив и обратный результат.

\textbf{Лемма.} Для того, чтобы матрица \(\mathbb{R}\) порядка
\(n \times n\) была ковариационной матрицей некоторого вектора
\(\xi = \left( \xi_1, \dots, \xi_n \right)\), необходимо и достаточно,
чтобы эта матрица была симметрицекой и неотрицательно определённой или,
что эквивалентно, существовала бы матрица \(A\) (порядка \(n \times k\),
\(1 \le k \le n\)) такая, что \( \mathbb{R} = AA^{T} \).

\[
    \mathrm{E} \xi \xi^T = \mathrm{E}(A\eta)(A\eta)^T = A \cdot \mathrm{E}\eta\eta^T \cdot A^T = AEA^T = AA^T
\]

    Сумма двух независимых гауссовских случайных величин снова есть
гауссовская случайная величина со средним \(m_1 + m_2\) и дисперсией
\(\sigma_1^2 + \sigma_2^2\).
