...
#
# **Утверждение.** Для любой матрицы $A$ с ненулевыми главными минорами найдётся такая невырожденная нижняя треугольная матрица $S$, что $SA$ есть верхняя треугольная матрица $U$ с единицами на главной диагонали:
# $$ SA = U. $$
#
...
#
# **Утверждение.** Невырожденную матрицу $A$ перестановкой строк (или столбцов) можно перевести в матрицу, главные миноры которой отличны от нуля.
#
# Тогда справедливо
# $$ PA = LU,$$
...
#
# **Утверждение.** Матрицу $A$, главные миноры которой не равны нулю, можно единственным образом разложить в произведение $L_1 D U$, в котором $D$ &mdash; диагональная, а $L_1$ и $U$ &mdash; нижняя и верхняя треугольные матрицы с единицами на главной диагонали.
...
#
# **Утверждение 1.** Нормальная система уравнений всегда совместна. \
...
# $$
#
# **Утверждение 2.** Точная нижняя грань квадрата нормы невязки достигается для всех решений нормальной системы и только для них.
#
# **Утверждение 3.** Нормальная система имеет единственное решение тогда и только тогда, когда столбцы матрицы $A$ линейно независимы.
#
...
#
# **Утверждение 4.** Если столбцы матрицы $A$ линейно независимы, то
# $$
#   A^+ = (A^\top A)^{-1} A^\top.
# $$
...
#
# **Утверждение 5.** Для любого столбца $\mathbf{y} \in \mathbb{R}_m$ столбец $A A^+ \mathbf{y}$ есть ортогональная проекция $\mathbf{y}$ на линейную оболочку столбцов матрицы $A$.
#
# **Утверждение 6.** Если $A = CR$ &mdash; скелетное разложение матрицы $A$, то её псевдообратная равна
# $$
#   A^+ = R^+ C^+ = R^\top (R R^\top)^{-1} (C^\top C)^{-1} C^\top.
# $$
#
# **Утверждение 7.** Если $A = U \Sigma V^\top$ &mdash; сингулярное разложение матрицы $A$, то $A^+ = V \Sigma^+ U^\top$. \
