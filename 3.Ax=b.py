...
# 1. $r_{AB}  \le r_A, r_B$,
# 1. $r_{A+B} \le r_A + r_B$,
# 1. $r_{A^\top A} = r_{AA^\top} = r_A = r_A^\top$,
# 1. Пусть $A: m \times r$, $B: r \times n$ и $r_A = r_B = r$, тогда $r_{AB} = r$.
#
# **Доказательства:**
#
...
# 1. Матрицы $A$ и $A^\top A$ имеют одно и то же ядро (доказать), поэтому их ранг одинаков.
# 1. Матрицы $A^\top A$ и $BB^\top$ невырождены, так как $r_{A^\top A} = r_{BB^\top} = r$. Их произведение, матрица $A^\top A BB^\top$, тоже невырождена и её ранг равен $r$. Отсюда $r = r_{A^\top A BB^\top} \le r_{AB} \le r_A = r$.

# >Как доказать, что $A$ и $A^\top A$ имеют одно и то же ядро? \
# >Если $Ax=0$, то $A^\top Ax = 0$. Поэтому $\mathbf{N}(A) \in \mathbf{N}(A^\top A)$. \
# >Если $A^\top Ax = 0$, то $x^\top A^\top Ax = |Ax|^2 = 0$. Поэтому $\mathbf{N}(A^\top A) \in \mathbf{N}(A)$.
...
# 1. Пространство столбцоы матрицы $A$ (образ), $\mathrm{dim} = r$
# 1. Нуль-пространство матрицы $A^\top$, $\mathrm{dim} = m-r$
#

# ### Альтернатива Фредгольма ###
#
# Для того чтобы система уравнений была совместна, необходимо и достаточно, чтобы каждое решение сопряженной однородной системы удовлетворяло уравнению $y^\top b = 0$.
#
# Основная теорема линейной алгебры часто формулируется в виде *альтернативы Фредгольма*: либо уравнение $Ax=b$ имеет решение при любой правой части, либо сопряжённое к нему однородное уравнение $A^\top y = 0$ имеет нетривиальное решение.
